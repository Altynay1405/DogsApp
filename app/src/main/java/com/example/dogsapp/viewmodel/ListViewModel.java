...
public class ListViewModel extends AndroidViewModel {
   ...
   private long refreshTime = 5 * 60 * 1000 * 1000 * 1000L;  //system clock works on nanoseconds - 5 min
    ...
    public void refresh() {
        checkCacheDuration();
        long updateTime  = prefHelper.getUpdatedTime(); //last time we updated the information
    ...
    private class InsertDogsTask extends AsyncTask<List<DogBreed>, Void, List<DogBreed>>{
        ...
        protected List<DogBreed> doInBackground(List<DogBreed>... lists) {
            ...
            List<Long> result = dao.insertAll(newList.toArray(new DogBreed[0])); //list of uuid
        ...
        @Override
        protected void onPostExecute(List<DogBreed> dogBreeds) {  //connect to foreground thread
    ...
    private class RetrieveDogsTask extends AsyncTask<Void, Void, List<DogBreed>>{
        ...
        protected void onPostExecute(List<DogBreed> dogBreeds) {
            dogsRetrieved(dogBreeds);
            Toast.makeText(getApplication(), "Dogs retrieved from the database", Toast.LENGTH_LONG).show();
        }
    }
}
